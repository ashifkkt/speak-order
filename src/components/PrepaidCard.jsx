    ...
    >
      <div className="flex flex-col h-full justify-between">
        <div className="flex-grow flex flex-col">
          ...
          {/* Features List */}
          <ul className={`space-y-2 ${isSelected ? 'text-[12px]' : 'text-[13.22px]'} ${isSelected ? 'leading-[16px]' : 'leading-[20px]'} font-outfit ${isSelected ? 'max-h-none' : ''}`}>
            {data?.subscriptions?.map((item, idx) => (
              <li key={idx} className={`flex items-start gap-2 ${isSelected ? 'py-0.5' : ''}`}>
                <div className="min-w-[20px] pt-0.5">
                  <input
                    ...
                    className={`w-4 h-4 cursor-pointer ${isSelected
                ...
                <div className="flex-1">
                  <label
                    htmlFor={`option-${idx}`}
                    className={`cursor-pointer ${isSelected ? 'text-white' : 'text-gray-900'}`}
                  >
                    {item?.display_name} : {item?.price}
                    <div
                      className={`text-[11px] ${isSelected ? "text-gray-300" : "text-gray-500"}`}
                    >
