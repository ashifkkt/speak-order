...
const DarkModeToggle = ({ isDarkMode, toggleDarkMode }) => {
  return (
    <button 
      ...
      className="p-2 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 fixed top-4 right-4 z-10 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
...
const App = () => {
  ...
  const handlePaymentFailure = (errorData) => {
    const errorMessage = errorData.details?.message || errorData.error || "Payment failed";
  ...
  const handleCardKeyboardInteraction = (planType) => {
    const planData = {
      type: planType,
      summary: planType === "Pay as You Go" ? { data: payAsPlan } : 
               planType === "Prepaid Bundle" ? { data: prepaidBundle } : 
               { data: monthlyPlan },
      planDetails: planType === "Pay as You Go" ? payAsPlan : 
                  planType === "Prepaid Bundle" ? prepaidBundle : 
                  monthlyPlan
    };
  ...
  const handlePrintInvoice = () => {
    ...
    printContent.innerHTML = `
      ...
      <div class="invoice-header">
        ...
        <p>Transaction Date: ${responseData.payment_date || new Date().toLocaleDateString()}</p>
      ...
      <div class="invoice-details">
        ...
        <table>
          ...
          ${responseData.plan_name ? `<tr>
            <th>Plan Name</th>
            <td>${responseData.plan_name}</td>
          </tr>` : ''}
          ${responseData.minutes ? `<tr>
            <th>Minutes</th>
            <td>${responseData.minutes}</td>
          </tr>` : ''}
          ${responseData.price ? `<tr>
            <th>Price</th>
        ...
        <table>
          ...
          ${responseData.transaction_id ? `<tr>
      ...
      <div className="animate-fadeIn">
        ...
        <div className="flex items-center justify-center mb-12 mt-6">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300">{paymentStatus.message}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden mb-12">
          <div className="px-6 py-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold dark:text-white">Subscription Details</h3>
              ...
              <div className="space-y-4">
                <div className="border-b dark:border-gray-700 pb-4">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="font-medium dark:text-white">{selectedPlan}</span>
                  </div>
                  
                  {responseData.plan_name && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-500 dark:text-gray-400">Plan Name</span>
                      <span className="font-medium dark:text-white">{responseData.plan_name}</span>
                    </div>
                  )}
                  ...
                  {responseData.minutes && (
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium dark:text-white">{responseData.minutes}</span>
                    </div>
                  )}
                  
                  {responseData.overage_rate && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-500 dark:text-gray-400">Overage Rate</span>
                      <span className="font-medium dark:text-white">{responseData.overage_rate}</span>
                    </div>
                  )}
                  ...
                  {responseData.price && (
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium dark:text-white">{responseData.price}</span>
                    </div>
                  )}
                </div>
                
                <div>
                  <p className="text-gray-500 dark:text-gray-400 mb-1">Subscription starts</p>
                  <p className="font-medium dark:text-white">
                    {responseData.activated_at ? new Date(responseData.activated_at).toLocaleDateString() : new Date().toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="border-b dark:border-gray-700 pb-4">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="font-medium text-xs break-all dark:text-white">{paymentStatus.paymentId}</span>
                  </div>
                  
                  {paymentStatus.orderId && (
                    <div className="flex justify-between mb-2">
                      <span className="text-gray-500 dark:text-gray-400">Order ID</span>
                      <span className="font-medium text-xs break-all dark:text-white">{paymentStatus.orderId}</span>
                    </div>
                  )}
                  ...
                  {responseData.transaction_id && (
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium text-xs break-all dark:text-white">{responseData.transaction_id}</span>
                    </div>
                  )}
                </div>
                
                <div>
                  <p className="text-gray-500 dark:text-gray-400 mb-1">Payment Date</p>
                  <p className="font-medium dark:text-white">
                    {responseData.payment_date ? new Date(responseData.payment_date).toLocaleString() : new Date().toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen w-full bg-white dark:bg-gray-900 dark:text-white font-Inter transition-colors duration-200">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center mb-10">
          <div className="mr-2">
            <svg className="w-8 h-8 dark:text-white" viewBox="0 0 24 24" fill="none">
        ...
        <div className="mt-12">
          {paymentStatus.isComplete ? (
            ...
            <>
              <h2 className="text-2xl font-bold mb-2" id="plan-selection-heading">Choose Your Plan</h2>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                  ...
                  <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-10" role="radiogroup" aria-labelledby="plan-selection-heading">
                    <div className="w-full flex justify-center animate-fadeIn" style={{animationDelay: '0.1s'}}>
                      <div 
                        className="w-full cursor-pointer" 
                        onClick={() => handleCardKeyboardInteraction("Pay as You Go")}
                        onKeyDown={(e) => {
                    ...
                    <div className="w-full flex justify-center animate-fadeIn" style={{animationDelay: '0.2s'}}>
                      <div 
                        className="w-full cursor-pointer"
                        onClick={() => handleCardKeyboardInteraction("Prepaid Bundle")}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') {
                            e.preventDefault();
                    ...
                    <div className="w-full flex justify-center animate-fadeIn" style={{animationDelay: '0.3s'}}>
                      <div 
                        className="w-full cursor-pointer"
                        onClick={() => handleCardKeyboardInteraction("Subscription")}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') {
                            e.preventDefault();
